...
DEFINE_bool(save_video, true, "save video");
DEFINE_int32(repeat_time, 0, "process repeat time");
DEFINE_string(data_path, "", "video path");
DEFINE_string(save_video_path, "", "save video path");
...
bool run() {
  ...
  try {
    ...
    g_decode = decode.get();
    if (FLAGS_save_video)
      writer.open(FLAGS_save_video_path.c_str(),CV_FOURCC('M', 'J', 'P', 'G'), 20, cv::Size(1280, 720), true);
